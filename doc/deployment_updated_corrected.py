    ...
    try:
        ...
        with Connection(
            ...
            if conn.run(f'test -d /opt/FaceFlick', warn=True, hide=False).failed:                
                ...
                print(message)
                logger.info(message)
                conn.sudo('mkdir -p /opt/FaceFlick', hide=False, password=server['password'])
            else:
            ...
            conn.sudo(f'mv {temp_path} {remote_path}', hide=False, password=server['password'])
            ...
            if install_data:
                ...
                conn.sudo(f'mv {temp_data_path} {remote_data_path}', hide=False, password=server['password'])
            ...
            conn.sudo('tar -xzvf /opt/FaceFlick.tar.gz -C /opt/', hide=False, password=server['password'])
            ...
            conn.sudo('rm -f /opt/FaceFlick.tar.gz', hide=False, password=server['password'])
            ...
            if install_data:
                ...
                conn.sudo('tar -xzvf /opt/FaceFlickData.tar.gz -C /opt/FaceFlick', hide=False, password=server['password'])
                ...
                conn.sudo('rm -f /opt/FaceFlickData.tar.gz', hide=False, password=server['password'])
            ...
            if install_driver:
                try:
                    ...
                    conn.sudo('cp /opt/FaceFlick/drivers/cuda-ubuntu2204.pin /etc/apt/preferences.d/cuda-repository-pin-600', hide=False, password=server['password'])
                    conn.sudo('dpkg -i /opt/FaceFlick/drivers/cuda-repo-ubuntu2204-11-8-local_11.8.0-520.61.05-1_amd64.deb', hide=False, password=server['password'])
                    print("Copied gpg key...")
                    conn.sudo('cp /var/cuda-repo-ubuntu2204-11-8-local/cuda-*-keyring.gpg /usr/share/keyrings/', hide=False, password=server['password'])
                    conn.sudo('apt-get update', hide=False, password=server['password'])
                    conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get -y install cuda', hide=False, password=server['password'])
                    ...
                    conn.sudo('dpkg -i /opt/FaceFlick/drivers/cudnn-local-repo-ubuntu2204-8.9.3.28_1.0-1_amd64.deb', hide=False, password=server['password'])
                    conn.sudo('cp /var/cudnn-local-repo-ubuntu2204-8.9.3.28/cudnn-local-*-keyring.gpg /usr/share/keyrings/', hide=False, password=server['password'])
                    print("Copied gpg key...")
                    conn.sudo('apt-get update', hide=False, password=server['password'])
                    conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -y libcudnn8 libcudnn8-dev', hide=False, password=server['password'])
            ...
            print(message)
            logger.info(message)
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt install -y python-is-python3', hide=False, password=server['password'])
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt install -y python3.10-venv', hide=False, password=server['password'])
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -y gcc', hide=False, password=server['password'])
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -y g++', hide=False, password=server['password'])
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -y python3-dev', hide=False, password=server['password'])
            conn.sudo('DEBIAN_FRONTEND=noninteractive apt-get install -y python3-tk', hide=False, password=server['password'])
                ...
                try:
                    ...
                    if exist .ok:
                        ...
                        if running.ok:
                            ...
                            conn.sudo('systemctl stop foreman', hide=False, password=server['password'])    
                        conn.sudo('systemctl disable foreman', hide=False, password=server['password'])
                      
                    # Then, move the service file to /etc/systemd/system with sudo
                    conn.sudo(f'mv {remote_service_file_path} /etc/systemd/system/foreman.service', hide=False, password=server['password'])
                    # Reload the systemd daemon
                    conn.sudo('systemctl daemon-reload', hide=False, password=server['password'])
                    # Enable the service
                    conn.sudo('systemctl enable foreman', hide=False, password=server['password'])
                except Exception as e:
                ...
                try:
                    ...
                    if exist .ok:
                        ...
                        if running.ok:
                            ...
                            conn.sudo('systemctl stop faceflickapi', hide=False, password=server['password'])    
                        conn.sudo('systemctl disable faceflickapi', hide=False, password=server['password'])
                    
                    # Then, move the service file to /etc/systemd/system with sudo
                    conn.sudo(f'mv {remote_service_file_path} /etc/systemd/system/faceflickapi.service', hide=False, password=server['password'])
                    # Reload the systemd daemon
                    conn.sudo('systemctl daemon-reload', hide=False, password=server['password'])
                    # Enable the service
                    conn.sudo('systemctl enable faceflickapi', hide=False, password=server['password'])
                    # Start the service
                    conn.sudo('systemctl start faceflickapi', hide=False, password=server['password'])
                except Exception as e:
                    message = f"Failed install service to {server['host']}: {e}"
                    print(message)
                    logger.info(message)
            ...
            if install_http:
                ...
                nginx_installed = conn.sudo('which nginx', warn=True, hide=True, password=server['password']).ok

                if nginx_installed:
                    print("Nginx is already installed. Stopping it...")
                else:
                    print("Nginx not found. Installing it...")
                    conn.sudo('apt update', hide=False, password=server['password'])
                    conn.sudo('DEBIAN_FRONTEND=noninteractive apt install -y nginx', hide=False, password=server['password'])

                conn.sudo('systemctl stop nginx', hide=False, password=server['password'])

                # Check if another service is running on port 80
                if conn.sudo('lsof -i :80', warn=True, hide=False, password=server['password']).ok:
                    message = "Another service is running on port 80..."
                    print(message)
                    logger.error(message)

                # Remove all existing configurations
                conn.sudo('rm -f /etc/nginx/sites-available/*', password=server['password'])
                conn.sudo('rm -f /etc/nginx/sites-enabled/*', password=server['password'])
                ...
                if conn.run(f'test -d {loading_point}', warn=True, hide=False).failed:                
                    conn.sudo(f'mkdir -p {loading_point}', hide=False, password=server['password'])
                ...
                if conn.run(f'test -f /etc/nginx/sites-available/images', warn=True, hide=False).failed:
                    conn.sudo(f'mv {httpconfig_file_path} /etc/nginx/sites-available/images', hide=False, password=server['password'])               
                ...
                if conn.run(f'test -L /etc/nginx/sites-enabled/images', warn=True, hide=False).failed:
                    conn.sudo('ln -sf /etc/nginx/sites-available/images /etc/nginx/sites-enabled/', password=server['password'])

                # Validate the configuration
                result = conn.sudo('nginx -t', warn=True, password=server['password'])
                if result.failed:
                ...
                conn.sudo('systemctl enable nginx', hide=False, password=server['password'])
                conn.sudo('systemctl start nginx', hide=False, password=server['password'])
            ...
            conn.sudo(f'mv {temp_path} {remote_config_path}', password=server['password'])
            ...
            if restart_service:
                ...
                if exist.ok:
                    ...
                    conn.sudo('systemctl restart foreman', hide=False, password=server['password'])                    
            ...
            if install_driver or install_service or force_restart:
                ...
                logger.info(message)
                try:
                    conn.sudo('reboot', hide=False, password=server['password'])
                except Exception as e:
